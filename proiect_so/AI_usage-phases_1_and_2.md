...
Tool-uri folosite: Claude-> pentru intelegerea folosirii functiei sigaction() si a mecanismului de semnale POSIX

Prompt 1: "Explain me how sigaction() function works and each field struct sigaction"

Clade a explicat ca sigaction este folosita pentru instalarea handlerelor de semnale in UNIX/POSIX si mi-a oferit signatura acestei functii. A prezentat structura si rolul fiecarui camp.
sa_handler → funcția apelată când apare semnalul;
sa_mask → semnalele blocate în timpul executării handlerului;
sa_flags → opțiuni suplimentare pentru comportamentul handlerului.
