...
//functie care citeste fisierul .monitor_pid si trimite semnalul
int notify_monitor()
{
  ...
  //citim pid ul 
  char buffer[16];//il punem intr un buffer codul de la pid
  memset(buffer,0,sizeof(buffer)); // curatam memoria pentru buffer
  int bytes_read = read(fd,buffer,sizeof(buffer) - 1); 
  //pentru a citi si a sti  cati biti am citit
  close(fd);
  ...
  pid_t pid_monitor = atoi(buffer);
  //am folosit pid_t in loc de int in cazul in care pid ul poate fi mai mare
  //pid_t si int sunt pt numere 
  if(pid_monitor<= 0)
  {
...
void writing_in_logged_district(int monitor_notified)
{
  ...
  char monitor_status[100] = "";
  if(strcmp(command,"add") == 0){
    if(monitor_notified == 1){
    strcpy(monitor_status,"-Monitor notified successfully");
    }
  
  else{
    strcpy(monitor_status,"-Monitor could not be informed");
  }
}
  char log_entry[MAX_LOG];//ce va aparea in prop finala
  snprintf(log_entry,MAX_LOG,"[%s] role = %s user = %s action = %s%s\n"
  ,time_str,role,user,command,monitor_status);
...
int main(int argc,char *argv[])
{
   ...
   if((strcmp(argv[i],"--filter") == 0) && i+2<argc)
  {
    ...
    break;//fara break imi da seg fault
  }
...
if(strlen(district) > 0 && strlen(command) > 0)
    {
     if(strcmp(command,"add") == 0) comanda_add(district,role,user);
    if(strcmp(command,"view") == 0) comanda_view(id_cautat);
    if(strcmp(command,"list") == 0) comanda_list();
    if(strcmp(command,"remove_report") == 0) comanda_remove();
    if(strcmp(command,"update_threshold") == 0) comanda_update_threshold();
    if(strcmp(command,"filter") == 0) comanda_filter(input); // trebuie pusa in " " conditia
    
    // Logăm totul, mai puțin ștergerea districtului
    if(strcmp(command, "remove_district") == 0) {
        comanda_remove_district();
    } else {
        int monitor_result = 0;
        if(strcmp(command,"add") == 0){
          monitor_result = notify_monitor();
        }
        writing_in_logged_district(monitor_result);
    }
  }
